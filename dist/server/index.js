...
const midi_service_1 = require("./midi-service");
const error_handler_1 = require("./middleware/error-handler");
const request_logger_1 = require("./middleware/request-logger");
const health_check_1 = require("./health/health-check");
const production_1 = __importStar(require("./config/production"));
const graceful_shutdown_1 = __importDefault(require("./shutdown/graceful-shutdown"));
const voice_1 = __importDefault(require("./routes/voice"));
const midi_1 = __importDefault(require("./routes/midi"));
const audioldm2_1 = __importDefault(require("./routes/audioldm2"));
...
const midiService = new midi_service_1.MidiService(); // MIDI generation and processing service
const healthChecker = health_check_1.HealthChecker.getInstance();
// PRODUCTION MIDDLEWARE CONFIGURATION
// NOTE: Middleware order is crucial - security first, then logging, then parsing
if (process.env.NODE_ENV === 'production') {
    app.use((0, helmet_1.default)(production_1.default.security.helmet));
    // Rate limiting
    const limiter = (0, express_rate_limit_1.default)(production_1.default.security.rateLimiting);
    app.use(limiter);
    // Start resource monitoring
    production_1.resourceMonitor.startMonitoring();
}
// Request logging (skip health checks to reduce noise)
app.use(request_logger_1.healthCheckLogger);
// CORS configuration
app.use((0, cors_1.default)(production_1.default.cors));
// Body parsing with enhanced limits and validation
app.use(express_1.default.json({
    limit: production_1.default.limits.json,
    verify: (req, res, buf) => {
        if (buf.length > production_1.default.upload.maxFileSize) {
            throw new error_handler_1.ApiError('Request payload too large', 413);
        }
    }
}));
app.use(express_1.default.urlencoded({
    extended: true,
    limit: production_1.default.limits.urlencoded,
    verify: (req, res, buf) => {
        if (buf.length > production_1.default.upload.maxFileSize) {
            throw new error_handler_1.ApiError('Request payload too large', 413);
        }
...
app.use((0, express_session_1.default)(auth_2.sessionConfig));
// Server timeout configuration
app.use((req, res, next) => {
    req.setTimeout(production_1.default.server.timeout, () => {
        res.status(408).json({
...
app.get('/api/health', health_check_1.healthCheckHandler);
...
app.get('/api/midi/:filename/metadata', async (req, res) => {
    try {
        ...
        const midiPath = (0, path_3.join)('./storage/midi/generated', filename);
...
app.use('/api/auth', auth_1.default); // Authentication and user management
app.use('/api/voice', voice_1.default); // Voice cloning and synthesis
app.use('/api/midi', midi_1.default); // MIDI generation and management
app.use('/api/audioldm2', audioldm2_1.default); // AI music generation
...
app.post('/api/generate-song', async (req, res) => {
    try {
        ...
        if (voiceSample) {
            const { RVCService } = await Promise.resolve().then(() => __importStar(require('./rvc-service')));
        ...
        if (useAI) {
            const { AudioLDM2Service } = await Promise.resolve().then(() => __importStar(require('./audioldm2-service')));
...
if (process.env.NODE_ENV === 'production') {
    server.setTimeout(production_1.default.server.timeout);
    server.keepAliveTimeout = production_1.default.server.keepAliveTimeout;
    server.headersTimeout = production_1.default.server.headersTimeout;
}
// Start health checks
...
const gracefulShutdown = new graceful_shutdown_1.default(server);
...
app.get('/test-ui', (req, res) => {
    res.sendFile(path_1.default.join(__dirname_compat, '../test-ui-interactions.html'));
});
