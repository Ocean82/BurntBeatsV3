...
function buildClient() {
  ...
  // Try approach 1: Use the main vite config if it exists
  if (existsSync('vite.config.ts')) {
    try {
      runCommand('npx vite build --outDir dist/public', 'Building with main vite config');
      ...
      log('⚠️ Main vite build failed, trying client directory...', 'warn');
    }
  }

  // Try approach 2: Build in client directory and copy
  if (existsSync('client') && existsSync('client/package.json')) {
    try {
      // Install client dependencies locally for build
      runCommand('cd client && npm install --include=dev', 'Installing client dependencies', { continueOnError: true });
      
      // Copy parent node_modules vite if needed
      if (!existsSync('client/node_modules/vite') && existsSync('node_modules/vite')) {
        try {
          cpSync('node_modules/vite', 'client/node_modules/vite', { recursive: true });
          cpSync('node_modules/@vitejs', 'client/node_modules/@vitejs', { recursive: true });
          log('📁 Copied vite from parent node_modules', 'info');
        } catch (copyError) {
          log('⚠️ Could not copy vite, continuing...', 'warn');
        }
      }
      
      runCommand('cd client && npx vite build --outDir ../dist/public', 'Building client in directory');
